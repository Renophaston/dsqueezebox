...
class Server:
    ...
    def __init__(self, servername='localhost', httpport=9000):
        ...
        self.jsonurl = self.get_jsonurl()

    # PUBLIC methods
    
    def query_server(self, *args):
        """Sends a JSON whatever to the server and returns the response as a
        JSONy dictionary.
    ...
    def get_id(self, playername):
        """Takes a name ("JohnsTouch") and returns the playerid or False
        if it can't find it."""
    ...
    def skip(self, playerid, amount):
        ...
        if amount != 0:
            self.query_server(playerid, 'playlist', 'index',
                              '{0:+}'.format(amount))
    
    def change_volume(self, playerid, amount):
        """Changes volume by amount (i.e. not _to_ amount)."""
        if amount != 0:
            self.query_server(playerid, 'mixer', 'volume',
                              '{0:+}'.format(amount))
        
    # PRIVATE methods
        
    def get_jsonurl(self):
        return 'http://' + self.servername + ':' + str(self.httpport) +\
               '/jsonrpc.js'
